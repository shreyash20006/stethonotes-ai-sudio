...
async function startServer() {
  ...
  app.post("/api/cashfree/create-session", async (req, res) => {
    ...
    // If credentials are not set, we can simulate a perfect session payload for development testing
    if (!appId || !secretKey) {
      return res.json({
        success: true,
        demoMode: true,
        ...
        message: "Running Cashfree in Demo Mode. Provide CASHFREE_APP_ID & CASHFREE_SECRET_KEY to connect your live dashboard."
      });
    }
    ...
    try {
      const response = await fetch(baseUrl, {
        ...
        body: JSON.stringify({
          ...
          order_meta: {
            return_url: `${req.headers.origin || "http://localhost:3000"}/api/cashfree/verify?order_id={order_id}`
          }
        })
      });
      ...
      if (!response.ok) {
        ...
        return res.status(response.status).json({
          success: false,
          error: errBody.message || "Failed to initiate Cashfree payment session."
        });
      }
